...
class AtmosphereModeling():
    ...
    def atmo_kurucz(self, starname, teff, logg, feh, vtur, erteff, erlogg,erfeh,ervtur):
        ...
        os.system('echo %s %s %s | intermod.e' % (teff, logg, feh))
        os.system('echo %s | transform.e' % vtur)
        os.system('mv out.atm %s_orig.atm' %starname)

        os.system('echo %s %s %s | intermod.e' % (teff, logg, feh + erfeh))
        os.system('echo %s | transform.e' % vtur)
        os.system('mv out.atm %s_erfeh.atm' %starname)

        os.system('echo %s %s %s | intermod.e' % (teff+erteff, logg, feh))
        os.system('echo %s | transform.e' % vtur)
        os.system('mv out.atm %s_erteff.atm' %starname)

        os.system('echo %s %s %s | intermod.e' % (teff, logg+erlogg, feh))
        os.system('echo %s | transform.e' % vtur)
        os.system('mv out.atm %s_erlogg.atm' %starname)

        os.system('echo %s %s %s | intermod.e' % (teff, logg, feh))
        os.system('echo %s | transform.e' % (vtur + ervtur))
